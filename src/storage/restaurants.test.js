import { extendRestaurantsData, getFilteredRestaurantsList } from './restaurants';
...
it('extends "storage.restaurants" data', () => {
  ...
  extendRestaurantsData(); // Actual call
...
it('filters "storage.restaurants" array depending on user input', () => {
  ...
  // 0 3 4 5 11
  let list = getFilteredRestaurantsList(defaultList);
  ...
  // 0 17 1
  list = getFilteredRestaurantsList(defaultList, 'ta');
  expect(list[0].id).toEqual(0);
  expect(list[1].id).toEqual(17);
  expect(list[2].id).toEqual(1);

  // 0  1 17
  list = getFilteredRestaurantsList(defaultList, 'ta', 3);
  expect(list[0].id).toEqual(0);
  expect(list[1].id).toEqual(1);
  expect(list[2].id).toEqual(17);

  // 0 7 5 3 9 - distance, empty search
  list = getFilteredRestaurantsList(defaultList, '', 4);
  expect(list[0].id).toEqual(0);
  ...
  // 4 15
  list = getFilteredRestaurantsList(list, '', 4);
